...
figure. **Verified self-contained** (Python 3.13, versions in §4).
...
the code. They are available **on request from the corresponding author** (and
intended for a public data repository, e.g. Zenodo, on publication —
`<TO FILL: DOI>`). Each cube is float32 `(N_samples, T, H, W)`; the figure's cube
`nlt4g32t05_mz_cube.npy` is `(1024, 201, 64, 64)`. Once obtained, pass its path
to the scripts via the `--cube` CLI argument (see §3, Tier 2). If you only need
to reproduce the figure or the MLP/CNN results, the bundled `.npz` caches are
sufficient and the cubes are not required.
...
Provenance: assembled from the source experiment folder. The original run log
is retained out-of-tree.
